# Asset Visualizations

<|{navbar}|raw|>

## Folder Size Treemap (by Asset Family)

<|chart|type=treemap|labels={labels}|parents={parents}|values={values}|>

<br/>

## Material Count Distribution

<|chart|type=histogram|x={materials}|nbins=20|>

<br/>

## Polycount Distribution

<|chart|type=histogram|x={polycounts}|nbins=40|>

<br/>

## Polycount vs Material Count (per Variant)

<|chart|type=scatter|x={scatter_x}|y={scatter_y}|text={scatter_text}|mode=markers|>

<br/>

## Heaviest Asset Families (Top 10)

<|chart|type=bar|x={leader_labels}|y={leader_values}|>
